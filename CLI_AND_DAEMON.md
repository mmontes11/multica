...
|----------|-------------|
| `MULTICA_<PROVIDER>_MAX_CONCURRENT_TASKS` | Per-provider concurrency cap (provider name uppercased, e.g. `MULTICA_CLAUDE_MAX_CONCURRENT_TASKS`). Unset = unlimited; a run whose provider is at its cap holds its machine slot and parks until a provider slot frees |
