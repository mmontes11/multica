...
Beyond the machine-wide cap, each provider can have its own through `MULTICA_<PROVIDER>_MAX_CONCURRENT_TASKS` (the runtime's provider name uppercased, e.g. `MULTICA_CLAUDE_MAX_CONCURRENT_TASKS=1` for a local GPU agent). A claimed run whose provider is at its cap holds its machine slot and parks until a slot for that provider frees, so the cap is enforced without re-queueing. The value is read the first time a run starts for that provider and stays in effect for the daemon's lifetime; providers without the variable are unlimited.
